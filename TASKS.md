...
| Storage v2 — deltas multi-segmento + flush O(delta) + compactação read-safe | not-started | Próximas unidades da ADR 0016: flush escreve segmento de delta (reusa ops do WAL) em vez de reescrever a base; query/aplicação multi-segmento; compactação online segura para leitores; testes crash entre fsync/rename/GC e read-durante-compactação; config multi-segmento no `differential_consistency.rs`. |
