...
| Tokenização de escritas sem espaço entre palavras (bigramas CJK) | validated | ADR 0017, `bm25::tokenizer_tests` (7 casos, inclui a prova de que escritas com espaço são byte-idênticas), `tests/python/test_unspaced_scripts.py`. Bug real: um run alfanumérico *é* a frase inteira em chinês/japonês/coreano/tailandês, então o índice tinha um único termo por chunk e **toda consulta por substring devolvia nada** — com `hybrid` padrão em todo preset, incluindo `multilingual`, degradava para dense-only em silêncio |
| Migração transparente do índice BM25 (`format_version = 3`) | validated | `pre_tokenizer_vault_rebuilds_bm25_on_reopen` — reconstrói do texto armazenado ao reabrir, preservando buracos no espaço de linhas e documentos deletados. Reusa o rebuild que a compactação já fazia |
| Um tokenizador só, compartilhado com o Python (`ragvault._native.tokenize`) | validated | `test_python_and_rust_agree_on_which_scripts_are_unspaced`. Bug real: `_token_set` usava `str.split()`, então dois chunks chineses quase idênticos tinham overlap 0.00 e o MMR os lia como "máxima diversidade" |
| Estimativa de tokens para escritas sem espaço | validated | `TestTokenEstimation`. Bug real: 35 caracteres chineses estimavam 4 tokens (5–9× baixo), então `target_tokens` e `token_budget` erravam pelo mesmo fator |
| Terminadores de sentença no chunking (mesma definição do verificador) | validated | `TestChunkSentenceSplitting`. Bug real: parágrafo chinês/híndi/árabe era uma sentença só, então parágrafo grande pulava a divisão e ia direto para hard-wrap, cortando no meio da frase |
| `effective_date` não-ISO não é forçada a um número que só parece ordenável | validated | `TestEffectiveDateParsing`. Bug real: `12/31/2023` superava `01/15/2024` — o documento substituído vencia por recência e entrava no contexto parecendo atual, anulando a supersessão absoluta. Agora ordena como desconhecida e é reportada em `result.unparseable_dates`; `date_format=` recupera a ordenação exata |
