...
| Verificador NLI offline (`ragvault.nli`) | implemented, **não medido** | `tests/python/test_nli.py` (33 testes com modelo stub: mapeamento por nome sob `id2label` embaralhado, granularidade de premissa, agregação, `uncited` sem chamada ao modelo). Acurácia **não medida**: `benchmarks/bench_nli_verifier.py` + dataset commitados, mas huggingface.co bloqueado neste ambiente → `RESULTS-VERIFICATION.md` publica ausência de números, não números inventados |
