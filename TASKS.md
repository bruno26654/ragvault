...
| Storage v2 — read-during-compaction sem bloqueio (snapshot pinning) | not-started | Único item restante da ADR 0016 (acceptance #4): hoje a compactação é síncrona sob o write lock (leitores bloqueiam brevemente mas nunca veem estado parcial — seguro). Concorrência sem bloqueio é otimização, não correção. |
